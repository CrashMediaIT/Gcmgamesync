...
Pass `--build` every time so Docker actually rebuilds the image instead of reusing a cached one — the bundled `docker-compose.yml` also sets `pull_policy: build` to make sure `compose up` never silently substitutes an older registry image.
...
> **Container fails to start with `Permission denied`?** The server runs as a non-root user (uid `10001` by default) and needs write access to `/data`. The shipped image now runs `docker-entrypoint.sh` as root on every boot, which auto-`chown`s `/data` to the runtime UID/GID before dropping privileges and exec'ing the binary — so a stale named volume or a host bind mount is repaired automatically the next time the container starts. If you have explicitly stripped `CHOWN`/`FOWNER`/`DAC_READ_SEARCH`/`SETUID`/`SETGID` from the container, either restore them or remove the stale volume with `docker compose down -v` and let the container recreate it.

> **Running on TrueNAS Scale, Unraid, Synology, or another NAS that owns the dataset as a different user?** Set the **`PUID`** and **`PGID`** environment variables on the container (TrueNAS Scale: *Apps → Custom App → Container Environment Variables*) to the uid/gid that owns the host directory you mounted at `/data` — for example `PUID=568` / `PGID=568` for the TrueNAS Scale `apps` user. The entrypoint will chown `/data` to that uid/gid and drop privileges to it instead of the default `10001:10001` baked into the image, so writes succeed without you having to chown the host dataset. The variables follow the same convention as linuxserver.io images and accept any non-negative integer.

> **Container crashes immediately and `docker compose logs` is empty?** First make sure you actually rebuilt the image — `docker compose up` without `--build` reuses whatever was cached, so any fix you applied may not be running yet. Run `docker compose up -d --build` and then tail logs with `docker logs --tail=200 crash-crafts-game-sync` (the container is named so `docker logs` works without looking up an ID) or `docker compose logs -f crash-crafts-game-sync`. The current build prints a startup banner (`crash-crafts-game-sync v… starting (pid …)`) on stderr before doing any work, plus per-step breadcrumbs (`preparing data directory …`, `binding TCP listener …`, `server listening on …`), so the very first line of the log will tell you which step crashed. The compose file also sets `restart: unless-stopped` and a bounded `json-file` log driver (`max-size: 10m`, `max-file: 5`) so logs survive crash/restart loops.
